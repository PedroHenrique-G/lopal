...
function mostrarAltitude(){
    ...
    contexto.fillText(
        `Altitude: ${(canvas.height - moduloLunar.posicao.y - 0.5 * moduloLunar.altura ).toFixed(2)}`,
        400,
        60
    );
}
...
function desenharChama(){
     contexto.beginPath();
    //define o ponto de origem da chama no canto inferior esquerda do módulo
    contexto.moveTo(moduloLunar.largura * -0.5, moduloLunar.altura * 0.5);
    //desenha uma linha até o ponto inferior direito fo módulo
    ...
    contexto.closePath();//desenha autormaticamente a linha do vértice à origem
...
function encerrarJogo(){
    if(moduloLunar.posicao.y + moduloLunar.altura * 0.5 > canvas.height ){
        if(moduloLunar.velocidade.y <= 0.5 && 
            ...
            mostrarResultado("Você passou com sucesso!", cor = "green");
        } else {
